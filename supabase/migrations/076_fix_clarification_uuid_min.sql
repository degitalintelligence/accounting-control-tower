...
BEGIN
  ...
  JOIN acct_ctrl.wa_groups AS incoming_group
    ON incoming_group.id = message.wa_group_id
  ...
  JOIN acct_ctrl.wa_messages AS source_message
    ON source_message.id::TEXT = suggestion_row.source_reference_id
  JOIN acct_ctrl.wa_groups AS source_group
    ON source_group.id = source_message.wa_group_id
  WHERE suggestion_row.organization_id = p_organization_id
    ...
    AND (
      suggestion_row.clarification_response_message_id IS NULL
      OR suggestion_row.clarification_response_message_id <> p_message_id
    );
  ...
  SET review_state = 'unclaimed',
      ...
      evidence_message_ids = CASE
        WHEN COALESCE(suggestion_update.evidence_message_ids, '[]'::jsonb)
          @> jsonb_build_array(p_message_id::TEXT)
        THEN COALESCE(suggestion_update.evidence_message_ids, '[]'::jsonb)
        ELSE COALESCE(suggestion_update.evidence_message_ids, '[]'::jsonb)
          || jsonb_build_array(p_message_id::TEXT)
      END,
      evidence_text = concat_ws(
        E'\n\n',
        NULLIF(suggestion_update.evidence_text, ''),
        'Jawaban klarifikasi: ' || left(COALESCE(incoming_message.content, ''), 4000)
      ),
      updated_at = now()
