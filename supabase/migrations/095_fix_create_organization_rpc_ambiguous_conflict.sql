-- Fix: create_organization_with_owner RPC gagal dengan error 42702
-- "column reference \"organization_id\" is ambiguous".
--
-- Akar masalah: fungsi ini `RETURNS TABLE(organization_id uuid, ...)` sehingga
-- `organization_id` menjadi kolom output (variabel PL/pgSQL). Pada klausa
-- `ON CONFLICT (organization_id, role_key)` dan
-- `ON CONFLICT (profile_id, organization_id, ...)`, PostgreSQL tidak bisa
-- membedakan `organization_id` sebagai kolom tabel vs variabel PL/pgSQL
-- sehingga selalu 500.
--
-- Perbaikan: pakai `ON CONFLICT ON CONSTRAINT <nama_index>` untuk menarget
-- constraint unik secara eksplisit tanpa me-refer kolom yang ambigu.
...
BEGIN
  ...
  ON CONFLICT ON CONSTRAINT organization_roles_organization_id_role_key_key DO NOTHING;
  ...
  ON CONFLICT ON CONSTRAINT memberships_profile_id_organization_id_client_id_entity_id__key
