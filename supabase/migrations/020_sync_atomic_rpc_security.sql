...
BEGIN
  ...
  IF p_maker_id IS NULL OR p_checker_id = p_maker_id THEN
    RAISE EXCEPTION 'Maker tidak valid atau maker dan checker harus berbeda';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Pembuat command tidak memiliki membership aktif pada scope client';
  END IF;

  IF NOT EXISTS (
    SELECT 1
    FROM acct_ctrl.memberships
    WHERE profile_id = p_maker_id
      AND organization_id = p_organization_id
      AND is_active
      AND (client_id IS NULL OR client_id = p_client_id)
  ) THEN
    RAISE EXCEPTION 'Maker tidak memiliki membership aktif pada scope client';
  END IF;

  IF p_checker_id IS NOT NULL AND NOT EXISTS (
    SELECT 1
    FROM acct_ctrl.memberships
    WHERE profile_id = p_checker_id
      AND organization_id = p_organization_id
      AND is_active
      AND (client_id IS NULL OR client_id = p_client_id)
  ) THEN
    RAISE EXCEPTION 'Checker tidak memiliki membership aktif pada scope client';
  END IF;
  ...
  INSERT INTO acct_ctrl.work_items (
    ...
    p_organization_id, p_client_id, 'ad_hoc', p_title, p_due_at,
    'whatsapp_command', p_source_reference_id::TEXT, p_source_metadata,
    'draft', p_created_by
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_work_items_whatsapp_source_reference
  ...
  WHERE source_type = 'whatsapp_command'
