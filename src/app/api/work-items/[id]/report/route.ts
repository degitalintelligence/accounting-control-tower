...
export async function PATCH(request: NextRequest, context: Context) {
  ...
  const memberships = await admin.from("memberships").select("client_id, role").eq("profile_id", user.id).eq("organization_id", organizationId).eq("is_active", true);
  const membershipData = memberships as unknown as { data: { client_id: string | null; role: string }[] | null };
  if (!membershipData.data?.some((entry) => (entry.client_id === null || entry.client_id === itemData.data!.client_id) && ["admin", "owner", "manager", "finance_manager", "accounting_manager"].includes(entry.role))) return NextResponse.json({ error: "Akses ditolak." }, { status: 403 });
