...
import { runNotificationOutboxWorker } from "@/lib/notification/outbox-worker";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await runNotificationOutboxWorker(createServiceRoleClient());
    return NextResponse.json({ data: result });
  } catch (error) {
    const details = error as { message?: string; code?: string; hint?: string; details?: string };
    console.error("[POST /api/jobs/notifications] Supabase error:", {
      message: details.message,
      code: details.code,
      hint: details.hint,
      details: details.details,
    });
    return NextResponse.json({ error: "Gagal menjalankan notification worker." }, { status: 500 });
  }
}
