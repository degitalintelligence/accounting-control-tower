...
export async function POST(_: Request, context: Context) {
  ...
  try {
    ...
    const organization = await requireActiveOrganization(auth.context.admin, auth.context.organizationId);
    const extraction = await extractTasksFromMessage(source, auth.context.locale, organization);
