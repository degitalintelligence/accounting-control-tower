...
export default function ClientsPage() {
  ...
  const canManage = has("clients.manage");
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
          <section className="surface-card overflow-hidden rounded-2xl">
            ...
            <div className="divide-y divide-slate-100">
              {loading ? <div className="flex items-center gap-2 px-5 py-10 text-sm text-slate-500"><Loader2 className="size-4 animate-spin" />Memuat daftar client...</div> : filteredClients.length === 0 ? <div className="px-5 py-12 text-center"><Building2 className="mx-auto size-10 text-slate-300" /><h3 className="mt-3 font-semibold text-slate-800">{clients.length ? "Client tidak ditemukan" : "Belum ada client"}</h3><p className="mt-1 text-sm text-slate-500">{clients.length ? "Coba ubah kata kunci pencarian." : "Tambahkan client pertama untuk mulai mengelola pekerjaan."}</p>{!clients.length && canManage && <Button onClick={startCreate} className="cta-primary mt-4"><Plus className="size-4" />Tambah client</Button>}</div> : filteredClients.map((client) => <div role="button" tabIndex={0} key={client.id} onClick={() => setSelectedId(client.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelectedId(client.id); } }} className={cn("flex w-full cursor-pointer items-center gap-3 px-5 py-4 text-left transition hover:bg-slate-50", selectedId === client.id && "bg-blue-50/60")}><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-blue-50 text-blue-600"><Building2 className="size-5" /></span><span className="min-w-0 flex-1"><strong className="block truncate text-sm font-semibold text-slate-900">{client.name}</strong><span className="mt-1 block truncate text-xs text-slate-500">{client.slug} · {client.timezone}</span></span><Badge variant="outline" className="hidden sm:inline-flex"><CheckCircle2 className="size-3 text-emerald-500" />Aktif</Badge>{canManage && <Button type="button" variant="ghost" size="icon-sm" onClick={(event) => { event.stopPropagation(); startEdit(client); }} aria-label={`Edit ${client.name}`}><Pencil /></Button>}</div>)}
            </div>
          </section>
          <aside className="space-y-6">
            {canManage && (
              <section className="surface-card rounded-2xl p-5 sm:p-6">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <h2 className="text-lg font-bold text-slate-950">{editingId ? "Edit client" : "Tambah client"}</h2>
                    <p className="mt-1 text-sm text-slate-500">Nama dan timezone dipakai sebagai konteks operasional.</p>
                  </div>
                  {editingId && (
                    <Button variant="ghost" size="icon-sm" onClick={startCreate} aria-label="Batalkan edit">
                      <X />
                    </Button>
                  )}
                </div>
                <form className="mt-5 space-y-4" onSubmit={saveClient}>
                  <div className="space-y-1.5">
                    <Label htmlFor="client-name">Nama client</Label>
                    <Input id="client-name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Contoh: PT Maju Jaya" required />
                  </div>
                  <p className="text-xs text-slate-500">Slug dibuat otomatis dari nama dan tidak perlu diisi manual.</p>
                  <div className="space-y-1.5">
                    <Label htmlFor="client-timezone">Timezone IANA</Label>
                    <Input id="client-timezone" value={form.timezone} onChange={(event) => setForm({ ...form, timezone: event.target.value })} placeholder="Asia/Jakarta" required />
                    <p className="text-xs text-slate-500">Gunakan format IANA, misalnya Asia/Jakarta atau Asia/Singapore.</p>
                  </div>
                  <div className="flex gap-2 pt-1">
                    <Button type="submit" disabled={saving} className="cta-primary">
                      {saving && <Loader2 className="animate-spin" />}
                      {editingId ? "Simpan perubahan" : "Buat client"}
                    </Button>
                    {editingId && (
            ...
            {canView && selectedClient ? (
              <section className="surface-card rounded-2xl p-5 sm:p-6">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-blue-600">Ringkasan client</p>
                    <h2 className="mt-1 text-lg font-bold text-slate-950">{selectedClient.name}</h2>
                  </div>
                  {canManage && (
                    <Button variant="ghost" size="icon-sm" onClick={() => setArchiveTarget(selectedClient)} aria-label={`Arsipkan ${selectedClient.name}`}>
                ...
                <dl className="mt-5 space-y-3 text-sm">
                  <div className="flex justify-between gap-4">
                    ...
                    <dd className="font-medium text-emerald-600">Aktif</dd>
                  </div>
                  <div className="flex justify-between gap-4">
                    <dt className="text-slate-500">Timezone</dt>
                    <dd className="font-medium text-slate-800">{selectedClient.timezone}</dd>
                  </div>
                  <div className="flex justify-between gap-4">
                    <dt className="text-slate-500">Slug</dt>
                    <dd className="max-w-[190px] truncate font-medium text-slate-800">{selectedClient.slug}</dd>
                  </div>
                  <div className="flex justify-between gap-4">
                    <dt className="text-slate-500">Dibuat</dt>
                    <dd className="font-medium text-slate-800">{new Intl.DateTimeFormat("id-ID", { dateStyle: "medium" }).format(new Date(selectedClient.created_at))}</dd>
                  </div>
                </dl>
                {canManage && (
                  <Button variant="outline" className="mt-5 w-full" onClick={() => startEdit(selectedClient)}>
              ...
              <section className="surface-card rounded-2xl p-5">
                ...
                <p className="mt-2 text-sm leading-6 text-slate-500">Pilih client untuk melihat detail ringkas dan aksi pengelolaan.</p>
              </section>
            )}
          </aside>
        </div>
