...
export default function RolesPage() {
  ...
  return <main className="page-canvas text-slate-900"><div className="mx-auto w-full max-w-6xl space-y-6"><header><div className="flex items-center gap-2 text-xs font-semibold text-blue-600"><KeyRound className="size-4" /> {t("settings.controlCenter")} <span className="text-slate-400">/</span> {t("settings.general")}</div><h1 className="mt-3 text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">{t("settings.rolesTitle")}</h1><p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500">{t("settings.rolesDescription")}</p></header><nav aria-label={t("common.breadcrumb")} className="flex max-w-full gap-2 overflow-x-auto border-b border-slate-200 pb-2">{settingsTabs.map((tab) => <Link key={tab.href} href={tab.href} className={cn("flex shrink-0 items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold", tab.href === "/settings/roles" ? "bg-blue-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100")}><tab.icon className="size-3.5" />{t(tab.labelKey as never)}</Link>)}</nav>{error && <div role="alert" className="flex items-center justify-between rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}<Button variant="outline" onClick={() => void load()}><RefreshCw className="size-4" /> {t("common.retry")}</Button></div>}{message && <div role="status" className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">{message}</div>}{loading ? <div className="h-80 animate-pulse rounded-2xl bg-white shadow-sm" /> : <div className="grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)]"><section className="surface-card rounded-2xl p-4"><div className="mb-3 flex items-center justify-between"><h2 className="font-bold">{t("settings.workspaceRoles")}</h2><Shield className="size-5 text-blue-600" /></div><div className="space-y-1">{roles.map((role) => <button key={role.id} type="button" onClick={() => choose(role)} className={cn("w-full rounded-lg px-3 py-2 text-left text-sm", selected === role.id ? "bg-blue-50 font-semibold text-blue-800" : "hover:bg-slate-50")}>{role.name}<span className="mt-0.5 block text-xs font-normal text-slate-500">{role.role_key}</span></button>)}</div><form onSubmit={createRole} className="mt-5 space-y-2 border-t border-slate-100 pt-4"><p className="text-sm font-bold">{t("settings.addCustomRole")}</p><input className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" placeholder={t("settings.roleKeyPlaceholder")} value={createForm.role_key} onChange={(event) => setCreateForm({ ...createForm, role_key: event.target.value })} /><input className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" placeholder={t("settings.roleNamePlaceholder")} value={createForm.name} onChange={(event) => setCreateForm({ ...createForm, name: event.target.value })} /><Button className="w-full cta-primary" disabled={saving}><Plus className="size-4" /> {t("settings.createRole")}</Button></form></section><section className="surface-card rounded-2xl p-5 sm:p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-lg font-bold">{current?.name ?? t("settings.selectRole")}</h2><p className="mt-1 text-sm text-slate-500">{current?.description ?? t("settings.permissionsDescription")}</p></div><Button onClick={() => void save()} disabled={saving || !current} className="cta-primary">{saving && <Loader2 className="size-4 animate-spin" />}{t("settings.saveChanges")}</Button></div>{current && <div className="mt-6 space-y-5">{Object.entries(grouped).map(([category, items]) => <div key={category}><h3 className="mb-2 text-sm font-bold text-slate-700">{category}</h3><div className="grid gap-2 sm:grid-cols-2">{items.map((permission) => <label key={permission.id} className="flex cursor-pointer gap-3 rounded-xl border border-slate-200 p-3 hover:border-blue-200"><input type="checkbox" checked={selectedPermissions.has(permission.id)} onChange={() => toggle(permission.id)} className="mt-1 size-4 accent-blue-600" /><span><span className="block text-sm font-semibold">{permission.name}</span><span className="block text-xs text-slate-500">{permission.description}</span></span></label>)}</div></div>)}</div>}</section></div>}</div></main>;
}
