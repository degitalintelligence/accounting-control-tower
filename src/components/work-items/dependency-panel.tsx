...
export function DependencyPanel({ workItemId }: { workItemId: string }) {
  ...
  return <div className="space-y-3"><div className="flex gap-2"><Input value={dependencyId} onChange={(event) => setDependencyId(event.target.value)} placeholder="ID work item prerequisite" /><Button onClick={add} disabled={busy || !dependencyId}><Link2 className="size-4" /> Tambah</Button></div>{items.length === 0 ? <p className="text-sm text-slate-400">Belum ada dependency.</p> : items.map((item) => <div key={item.id} className="flex items-center gap-2 rounded-lg border border-slate-100 p-3"><div className="flex-1"><p className="text-sm font-medium">{item.depends_on?.title ?? item.depends_on_id}</p><p className="text-xs text-slate-500">{item.dependency_type} · {item.depends_on?.status ?? ""}</p></div><Button variant="ghost" size="icon-sm" disabled={busy} onClick={() => remove(item.id)}>{busy ? <Loader2 className="size-4 animate-spin" /> : <X className="size-4 text-red-500" />}</Button></div>)}{error && <p className="text-sm text-red-600">{error}</p>}</div>;
}
