...
const decisionLabels = { approved: "Setujui", rejected: "Tolak", revision_required: "Minta Revisi" };

function ReviewEntry({ review }: { review: ReviewRecord }) {
  return (
    <div className="rounded-lg border border-slate-200 p-3 space-y-2">
      <div className="flex items-center justify-between gap-2">
        ...
        <Badge className={review.decision === "approved" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}>
          {review.decision ? decisionLabels[review.decision] : "Draft"}
        </Badge>
      </div>
      ...
      <p className="text-[11px] text-slate-400">{new Date(review.created_at).toLocaleString("id-ID")}</p>
    </div>
  );
}

function ApprovalEntry({ approval }: { approval: ApprovalRecord }) {
  return (
    <div className="rounded-lg border border-slate-200 p-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <Badge className={approval.decision === "approved" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}>
          {approval.decision ? decisionLabels[approval.decision] : "Draft"}
        </Badge>
      </div>
      {approval.comment && <p className="mt-2 text-sm text-slate-600 whitespace-pre-wrap">{approval.comment}</p>}
      <p className="mt-2 text-[11px] text-slate-400">{new Date(approval.created_at).toLocaleString("id-ID")}</p>
    </div>
  );
}
        ...
        <CardContent className="space-y-3">
          {actionable ? (
            <>
              ...
              <textarea value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Catatan keputusan" className="min-h-20 w-full rounded-md border border-slate-200 p-3 text-sm outline-none focus:ring-2 focus:ring-blue-500" />
              {kind === "review" && <textarea value={finding} onChange={(event) => setFinding(event.target.value)} placeholder="Finding (opsional)" className="min-h-16 w-full rounded-md border border-slate-200 p-3 text-sm outline-none focus:ring-2 focus:ring-blue-500" />}
              {error && <p className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p>}
              <Button onClick={submit} disabled={saving} className="bg-orange-500 text-white hover:bg-orange-600 font-bold"><CheckCircle2 className="size-4" />{saving ? "Menyimpan..." : kind === "approval" ? "Simpan Approval" : "Simpan Review"}</Button>
            </>
          ) : <p className="text-sm text-slate-500 flex items-center gap-2"><MessageSquareWarning className="size-4" />Belum ada tindakan yang tersedia untuk role/status Anda.</p>}
        </CardContent>
      </Card>
      <Card>
        <CardHeader className="pb-3"><CardTitle className="flex items-center justify-between gap-2 text-sm"><span className="flex items-center gap-2"><Sparkles className="size-4 text-blue-600" />AI Notes</span><Button variant="outline" size="sm" onClick={generateAiNote} disabled={aiLoading}>{aiLoading ? <Loader2 className="size-4 animate-spin" /> : <Sparkles className="size-4" />}Buat catatan</Button></CardTitle></CardHeader>
        <CardContent className="space-y-3">
          {aiNotes.length === 0 ? <p className="text-sm text-slate-500">Belum ada catatan AI. Catatan hanya membantu reviewer dan tidak mengubah keputusan.</p> : aiNotes.map((note) => <div key={note.id} className="rounded-lg border border-slate-200 p-3 space-y-2"><div className="flex items-center justify-between gap-2"><Badge className={note.status === "accepted" ? "bg-emerald-50 text-emerald-700" : note.status === "rejected" ? "bg-red-50 text-red-700" : "bg-amber-50 text-amber-700"}>{note.status === "accepted" ? "Diterima manager" : note.status === "rejected" ? "Ditolak manager" : "Menunggu keputusan manager"}</Badge><span className="text-[11px] text-slate-400">{new Date(note.created_at).toLocaleString("id-ID")}</span></div><p className="text-sm text-slate-700">{note.result.summary}</p>{[...note.result.completeness, ...note.result.anomalies].map((finding, index) => <div key={`${note.id}-${index}`} className="rounded-md bg-slate-50 px-3 py-2 text-sm"><span className="font-medium text-slate-900">{finding.area}</span><span className="text-slate-600"> — {finding.finding}</span></div>)}{note.result.recommendations.length > 0 && <p className="text-sm text-slate-600">Saran: {note.result.recommendations.join("; ")}</p>}{note.status === "pending" && ["admin", "manager", "finance_manager", "accounting_manager"].includes(membershipRole ?? "") && <div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => decideAiNote(note.id, "accept")}><ThumbsUp className="size-4" />Terima</Button><Button variant="outline" size="sm" onClick={() => decideAiNote(note.id, "reject")}><ThumbsDown className="size-4" />Tolak</Button></div>}</div>)}
        </CardContent>
      </Card>
      {data.reviews.length > 0 && <Card><CardHeader className="pb-3"><CardTitle className="text-sm">Riwayat Review</CardTitle></CardHeader><CardContent className="space-y-3">{data.reviews.map((review) => <ReviewEntry key={review.id} review={review} />)}</CardContent></Card>}
      {data.approvals.length > 0 && <Card><CardHeader className="pb-3"><CardTitle className="text-sm">Riwayat Approval</CardTitle></CardHeader><CardContent className="space-y-3">{data.approvals.map((approval) => <ApprovalEntry key={approval.id} approval={approval} />)}</CardContent></Card>}
    </div>
  );
}
