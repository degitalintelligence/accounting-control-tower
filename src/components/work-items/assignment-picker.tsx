...
export function AssignmentPicker({ workItemId, assignments, onChanged }: { workItemId: string; assignments: Assignment[]; onChanged: () => void }) {
  ...
  const canManage = has("work_items.manage");

  useEffect(() => {
    if (canManage) {
      fetch("/api/assignment-members")
  ...
  return <div className="space-y-3">
    {assignments.map((assignment) => (
      <div key={assignment.id} className="flex items-center gap-2 rounded-lg border border-slate-100 p-2">
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-medium text-slate-800">{assignment.profile_name ?? assignment.profile_id}</p>
          <p className="text-xs text-slate-500">{assignment.role}</p>
        </div>
        {canManage && (
          <Tooltip>
            <TooltipTrigger render={<Button variant="ghost" size="icon-sm" disabled={busy} onClick={() => remove(assignment)} aria-label={`Hapus assignment ${assignment.profile_name ?? assignment.profile_id}`} />}>
      ...
      <>
        <div className="grid gap-2 sm:grid-cols-[1fr_140px_auto]">
          <Select value={profileId} onValueChange={(value) => setProfileId(value ?? "")}>
            ...
            <SelectContent>{members.map((member) => <SelectItem key={member.profile_id} value={member.profile_id}>{member.name}</SelectItem>)}</SelectContent>
          </Select>
          ...
          <Button onClick={assign} disabled={busy || !profileId}>{busy ? <Loader2 className="size-4 animate-spin" /> : <UserPlus className="size-4" />} Assign</Button>
        </div>
