...
export function AssignmentPicker({ workItemId, assignments, onChanged }: { workItemId: string; assignments: Assignment[]; onChanged: () => void }) {
  ...
  return <div className="space-y-3">
    {assignments.map((assignment) => <div key={assignment.id} className="flex items-center gap-2 rounded-lg border border-slate-100 p-2"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-slate-800">{assignment.profile_name ?? assignment.profile_id}</p><p className="text-xs text-slate-500">{assignment.role}</p></div><Button variant="ghost" size="icon-sm" disabled={busy} onClick={() => remove(assignment)}><X className="size-4 text-red-500" /></Button></div>)}
