...
export function WhatsAppWhitelistWizard(props: Props) {
  ...
  return <section className="surface-card rounded-2xl p-5 sm:p-6">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-lg font-bold text-slate-950">{t("whatsapp.whitelist")}</h2><p className="mt-1 max-w-2xl text-sm text-slate-500">{t("whatsapp.whitelistDescription")}</p></div><div className="flex items-center gap-2"><MessageCircle className="size-5 text-blue-600" /><Button className="cta-primary" disabled={!props.canManage} onClick={() => { props.setSession(""); setSessionDialogOpen(true); }}><Plus className="size-4" />Tambah session</Button></div></div>
    <div className="mt-5 space-y-2">{props.data.connections.length ? props.data.connections.map((item) => { const retired = item.status === "retired"; const working = ["working", "connected", "healthy"].includes(item.status.toLowerCase()); return <div key={item.id} className={`flex flex-wrap items-center justify-between gap-3 rounded-xl border p-3 ${props.selected === item.id ? "border-blue-300 bg-blue-50/50" : "border-slate-200"}`}><button type="button" onClick={() => props.onSelect(item.id)} className="min-w-48 flex-1 text-left"><span className="block font-semibold">{item.session_id || "Session tanpa nama"}</span><span className="mt-1 block text-xs text-slate-500">{item.provider} · {retired ? "Retired / histori tersimpan" : item.last_health_check_at ? `Dicek ${new Date(item.last_health_check_at).toLocaleString("id-ID")}` : "Belum dicek"}</span></button><Status value={item.status} /><div className="flex flex-wrap gap-2">{!retired && <>{!working && <Button size="sm" variant="outline" onClick={() => props.onQr(item)} disabled={!props.canManage || props.busy === `qr-${item.id}`}><QrCode className="size-4" />QR</Button>}{working ? <Button size="sm" variant="outline" onClick={() => props.onStop(item.id)} disabled={!props.canManage || props.busy === `stop-${item.id}`}><Wifi className="size-4" />Putuskan</Button> : <Button size="sm" variant="outline" onClick={() => props.onStart(item.id)} disabled={!props.canManage || props.busy === `start-${item.id}`}><Wifi className="size-4" />Mulai ulang</Button>}<Button size="sm" variant="ghost" onClick={() => props.onHealth(item.id)} disabled={!props.canManage || props.busy === `health-${item.id}`}><RefreshCw className="size-4" />Cek status</Button><Button size="sm" variant="outline" onClick={() => { props.onSelect(item.id); setStep(1); }}>Kelola koneksi</Button><Button size="sm" variant="destructive" disabled={!props.canManage} onClick={() => props.onRetire(item)}><XCircle className="size-4" />Hapus</Button></>}{retired && <Button size="sm" variant="destructive" onClick={() => props.onArchive(item)} disabled={!props.canManage || props.busy === `archive-${item.id}`}><XCircle className="size-4" />Hapus dari daftar</Button>}</div></div>; }) : <p className="rounded-xl border border-dashed border-slate-200 px-4 py-8 text-center text-sm text-slate-500">Belum ada session WhatsApp.</p>}</div>
    <Dialog open={sessionDialogOpen} onOpenChange={setSessionDialogOpen}><DialogContent><DialogHeader><DialogTitle>Tambah session WhatsApp</DialogTitle><DialogDescription>Masukkan ID session WAHA untuk memulai koneksi nomor operasional.</DialogDescription></DialogHeader><div className="space-y-3"><Input aria-label="ID sesi WAHA" placeholder="Masukkan nama session" value={props.session} onChange={(event) => props.setSession(event.target.value)} /><Button onClick={() => { props.onCreate(); setSessionDialogOpen(false); }} disabled={props.busy === "create" || !props.session.trim()} className="cta-primary w-full">{props.busy === "create" ? <Loader2 className="size-4 animate-spin" /> : <Plus className="size-4" />}Buat session</Button></div></DialogContent></Dialog>
    {props.selected && <><nav aria-label={t("whatsapp.whitelist")} className="mt-6 grid gap-2 sm:grid-cols-4">{steps.slice(1).map(([value, label, Icon], index) => { const actualIndex = index + 1; return <button type="button" key={value} onClick={() => actualIndex <= step && setStep(actualIndex)} className={`flex items-center gap-2 rounded-xl border px-3 py-3 text-left text-sm ${actualIndex === step ? "border-blue-300 bg-blue-50 text-blue-900" : actualIndex < step ? "border-emerald-200 bg-emerald-50 text-emerald-900" : "border-slate-200 text-slate-500"}`}><span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-white text-xs font-bold shadow-sm">{actualIndex < step ? <Check className="size-4" /> : actualIndex + 1}</span><span className="font-semibold">{label}</span><Icon className="ml-auto hidden size-4 sm:block" /></button>; })}</nav><div className="mt-6 min-h-72">
      {step === 1 && <div className="space-y-5"><div className="rounded-xl border border-emerald-100 bg-emerald-50 p-4"><p className="font-semibold text-emerald-950">2. Pilih grup yang boleh diproses</p><p className="mt-1 text-sm leading-6 text-emerald-800">Hanya grup yang diaktifkan di sini yang dapat masuk ke sistem.</p></div><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-semibold">{connection?.session_id || "Pilih koneksi terlebih dahulu"}</p><p className="text-sm text-slate-500">{groups.length} grup terdaftar pada koneksi ini.</p></div><Button variant="outline" onClick={props.onDiscoverGroups} disabled={!props.canManage || !connection || props.busy === "discover-groups"}>{props.busy === "discover-groups" ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />} Muat grup dari WAHA</Button></div>{props.discoveredGroups.length > 0 && <SearchableGroupSelect groups={props.discoveredGroups} value={props.providerGroupId} getId={discoveredGroupId} getName={discoveredGroupName} onChange={(value) => { props.setProviderGroupId(value); const found = props.discoveredGroups.find((item) => discoveredGroupId(item) === value); props.setGroupName(found ? discoveredGroupName(found) : ""); void props.onDiscoverParticipants(value); }} />}<div className="grid gap-3 sm:grid-cols-2"><Input aria-label="Nama grup WhatsApp" placeholder="Nama grup" value={props.groupName} onChange={(event) => props.setGroupName(event.target.value)} /><select aria-label="Cakupan client grup" value={props.selectedClientId ?? ""} onChange={(event) => props.onClientChange(event.target.value || null)} className="h-10 rounded-md border border-slate-200 bg-white px-3 text-sm"><option value="">Seluruh organisasi</option>{props.clients.map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select></div><div className="flex flex-wrap gap-2"><Button onClick={props.onAddGroup} disabled={!props.canManage || !connection || !props.providerGroupId.trim() || props.busy === "group"} className="cta-primary"><Plus className="size-4" /> Aktifkan grup</Button><Button variant="outline" onClick={next} disabled={groups.length === 0}>Lanjut ke kontak <ChevronRight className="size-4" /></Button></div><div className="grid gap-2 sm:grid-cols-2">{groups.map((group) => <div key={group.id} className={`rounded-xl border p-4 ${group.id === props.groupId ? "border-blue-300 bg-blue-50" : "border-slate-200"}`}><button type="button" onClick={() => props.setGroupId(group.id)} className="w-full text-left"><span className="font-semibold">{group.group_name || group.provider_group_id}</span><span className="mt-1 block text-xs text-slate-500">{groupClientName(group.client_id)} · {group.is_active ? "Aktif" : "Nonaktif"}</span></button>{group.is_active ? <Button size="sm" variant="destructive" className="mt-3" onClick={() => props.onDeactivateGroup(group)} disabled={!props.canManage || props.busy === `deactivate-group-${group.id}`}><XCircle className="size-4" /> {props.busy === `deactivate-group-${group.id}` ? "Memproses..." : "Nonaktifkan grup"}</Button> : <Button size="sm" variant="outline" className="mt-3" onClick={() => props.onActivateGroup(group)} disabled={!props.canManage || props.busy === `activate-group-${group.id}`}><Check className="size-4" /> {props.busy === `activate-group-${group.id}` ? "Mengaktifkan..." : "Aktifkan kembali"}</Button>}</div>)}</div></div>}
      {step === 2 && <div className="space-y-5"><div className="rounded-xl border border-violet-100 bg-violet-50 p-4"><p className="font-semibold text-violet-950">3. Petakan kontak ke anggota aplikasi</p><p className="mt-1 text-sm leading-6 text-violet-800">Pilih identitas yang sudah diverifikasi. Nama ambigu tidak boleh ditebak.</p></div><select aria-label="Grup untuk pemetaan" value={props.groupId} onChange={(event) => { const groupId = event.target.value; const selected = groups.find((group) => group.id === groupId); props.setGroupId(groupId); props.setParticipantId(""); props.setParticipantName(""); props.setParticipantProfile(""); void props.onDiscoverParticipants(selected?.provider_group_id ?? ""); }} className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm"><option value="">Pilih grup aktif</option>{groups.filter((group) => group.is_active).map((group) => <option key={group.id} value={group.id}>{group.group_name || group.provider_group_id}</option>)}</select>{props.busy === "discover-participants" && <p className="text-sm text-slate-500">Memuat kontak dari grup...</p>}{availableParticipants.length > 0 && <select aria-label="Kontak WhatsApp ditemukan" value={props.participantId} onChange={(event) => { props.setParticipantId(event.target.value); const found = props.discoveredParticipants.find((item) => discoveredValue(item.id) === event.target.value || discoveredValue(item.lid) === event.target.value); props.setParticipantName(found?.displayName ?? found?.name ?? found?.phone ?? ""); }} className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm"><option value="">Pilih kontak dari WAHA</option>{availableParticipants.map((item) => { const id = discoveredValue(item.id) || discoveredValue(item.lid) || item.phone || ""; return <option key={id} value={id}>{item.displayName ?? item.name ?? item.phone ?? id}</option>; })}</select>}{props.groupId && !props.busy && availableParticipants.length === 0 && <p className="text-sm text-amber-600">Kontak grup belum tersedia atau gagal dimuat. Coba pilih grup lagi.</p>}<div className="grid gap-3 sm:grid-cols-2"><div className="flex h-10 items-center rounded-md border border-slate-200 bg-slate-50 px-3 text-sm text-slate-600">{props.participantId || "Kontak belum dipilih"}</div><select aria-label="Anggota aplikasi" value={props.participantProfile} onChange={(event) => props.setParticipantProfile(event.target.value)} className="h-10 rounded-md border border-slate-200 bg-white px-3 text-sm"><option value="">Pilih anggota aplikasi</option>{props.members.map((member) => <option key={member.profile_id} value={member.profile_id}>{member.name}</option>)}</select></div><Input aria-label="Nama kontak" placeholder="Nama kontak (opsional)" value={props.participantName} onChange={(event) => props.setParticipantName(event.target.value)} /><div className="flex flex-wrap gap-2"><Button onClick={props.onAddMapping} disabled={!props.canManage || !props.groupId || !props.participantId || !props.participantProfile || props.busy === "mapping"} className="cta-primary"><Plus className="size-4" /> Verifikasi kontak</Button><Button variant="outline" onClick={next} disabled={mappings.length === 0}>Lanjut ke review <ChevronRight className="size-4" /></Button></div><div className="space-y-2">{mappings.map((mapping) => <div key={mapping.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-emerald-100 bg-emerald-50 p-3 text-sm"><span>{mapping.display_name || mapping.provider_participant_id} · {mapping.is_verified ? "Terverifikasi" : "Menunggu verifikasi"}</span>{mapping.is_verified ? <Button size="sm" variant="destructive" onClick={() => props.onUnverifyMapping(mapping)} disabled={!props.canManage || props.busy === `unverify-mapping-${mapping.id}`}><XCircle className="size-4" /> {props.busy === `unverify-mapping-${mapping.id}` ? "Memproses..." : "Batalkan verifikasi"}</Button> : <Button size="sm" variant="outline" onClick={() => { setStep(2); props.onVerifyMapping(mapping); }} disabled={!props.canManage || props.busy === `verify-mapping-${mapping.id}`}><Check className="size-4" /> {props.busy === `verify-mapping-${mapping.id}` ? "Memverifikasi..." : "Verifikasi ulang"}</Button>}</div>)}</div></div>}
