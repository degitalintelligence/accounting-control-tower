...
import { useWaInbox, type WaInboxItemData } from "@/hooks/use-wa-inbox";
...
function WaActionCard({ action, onConfirm, onReject }: { action: WaInboxItemData; onConfirm: (id: string, clientId?: string) => void; onReject: (id: string) => void }) {
  return <div className="border border-[#eef0ee] rounded-lg p-3 space-y-2.5">
    ...
    <div className="flex items-center gap-1.5 pt-0.5"><button onClick={() => onReject(action.id)} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-[#dfe4e1] text-[10px] font-medium text-[#8a9490] hover:bg-[#f3f5f2]"><X className="w-3 h-3" />Dismiss</button><button onClick={() => onConfirm(action.id)} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-[#20865a] text-[10px] font-bold text-white hover:bg-[#1a6e4a] ml-auto"><Check className="w-3 h-3" />Confirm task</button></div>
  </div>;
}
