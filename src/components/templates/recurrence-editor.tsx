...
type Preview = { date: string; adjusted_from: string | null };
type Rule = { id?: string; rrule: string; timezone: string; generation_lead_days: number; holiday_handling: string; skip_weekends: boolean };
...
export function RecurrenceEditor({ templateId }: { templateId: string }) {
  const [mode, setMode] = useState<Mode>("daily");
  const [interval, setInterval] = useState("1");
  const [days, setDays] = useState<string[]>(["MO"]);
  const [monthDay, setMonthDay] = useState("1");
  const [month, setMonth] = useState("1");
  const [custom, setCustom] = useState("FREQ=DAILY;INTERVAL=1");
