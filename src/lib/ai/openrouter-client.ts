...
export function validateWhatsAppSummary(value: unknown): WhatsAppSummaryResult {
  ...
  const allowedTopicClassifications = new Set(["fact", "decision", "task", "update", "question", "blocker", "request", "reference", "noise"]);
  const topics = Array.isArray(value.topics) ? value.topics.filter(isRecord).map((topic) => ({ key: limitText(String(topic.key ?? "general"), 120), title: limitText(String(topic.title ?? "Topik percakapan"), 240), summary: limitText(String(topic.summary ?? ""), 600), classifications: Array.isArray(topic.classifications) ? topic.classifications.filter((entry): entry is string => typeof entry === "string" && allowedTopicClassifications.has(entry)).slice(0, 9) : [], message_ids: Array.isArray(topic.message_ids) ? topic.message_ids.filter((entry): entry is string => typeof entry === "string").slice(0, 100) : [] })) : [];
