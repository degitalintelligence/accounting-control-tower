...
describe("validasi policy administrasi", () => {
  it("menerima rule escalation yang kompatibel dengan engine", () => {
    const result = validateEscalationRules([
      { threshold_hours: 24, level: "team_lead", priority: "high", recipient_roles: ["manager"] },
  ...
  it.each([
    { name: "threshold tidak meningkat", rules: [{ threshold_hours: 48, level: "team_lead", priority: "high" }, { threshold_hours: 24, level: "owner", priority: "critical" }], message: "threshold_hours harus meningkat" },
    { name: "level menurun", rules: [{ threshold_hours: 24, level: "owner", priority: "high" }, { threshold_hours: 48, level: "team_lead", priority: "critical" }], message: "level eskalasi tidak boleh menurun" },
